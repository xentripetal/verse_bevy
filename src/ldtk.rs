        ...
        Box::pin(async move {
            ...
            if project.external_levels {
                ...
                for level in project.levels.iter() {
                    futures.push(Box::pin(load_context.read_asset_bytes(
                        load_context.path().parent().unwrap().join(
                            level.external_rel_path.as_ref().ok_or(anyhow!(
                                "Invalid ldtk file, expected external_rel_path but none found"
                            ))?,
                        ),
                    )));
                }
                ...
                for level_bytes in try_join_all(futures).await? {
                    let external_level: ldtk_rust::Level =
                        serde_json::from_slice(level_bytes.as_slice())?;
...
/**
* Initializes or removes Levels for worlds as they are created or updated
**/
pub fn initialize_ldtk_projects(
    mut commands: Commands,
    mut map_events: EventReader<AssetEvent<LdtkMap>>,
    new_worlds: Query<&Handle<LdtkMap>, Added<Handle<LdtkMap>>>,
    mut worlds: Query<(Entity, &Handle<LdtkMap>, Option<&Children>)>,
    mut levels: Query<(Entity, &Level, &Handle<LdtkMap>)>
) {
    let mut changed_maps = Vec::<Handle<LdtkMap>>::default();
    // If a new World is added, create all its children Levels
    // If a LdtkMap is changed, refresh its levels
}
...
pub fn process_loaded_tile_maps(
    ...
    mut levels: Query<(Entity, &Level, &mut Transform, &mut Map)>,
    mut maps: Query<(Entity, &Handle<LdtkMap>, &Children)>,
    ...
    // I feel like theres a better way to isolate what actually changed
    // so we don't regenerate the entire map every time.
    for changed_map in changed_maps.iter() {
        for (map_entity, map_handle, children) in maps.iter_mut() {
            ...
            if let Some(ldtk_map) = ldtk_maps.get(map_handle) {
                ...
                // Spawn each level
                for level in &ldtk_map.project.levels {
                    ...
                    let mut map = Map::new(0u16, level_entity);

                    for (layer_id, ldtk_layer) in level
                        .layer_instances
                        .as_ref()
                        .unwrap()
                        .iter()
                        ...
                        for tile in ldtk_layer.auto_layer_tiles.iter() {
                            ...
                            layer_builder
                                .set_tile(
                                    pos,
                                    Tile {
                                        texture_index: (tileset_y * tileset_width_in_tiles
                                            + tileset_x)
                                            as u16,
                                        flip_x: tile.f & 1 == 1,
                                        flip_y: tile.f & 2 == 2,
